...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET' && action === 'clickup-live') {
    ...
    const isTaskDone  = (t) => cuTaskDone(t);
    const getStatusCat = (t) => cuTaskStatusCat(t);
    ...
    const members = usersRes.rows.map((user) => {
      ...
      // Usa getStatusCat (padrão amplo) para detectar tasks concluídas no ClickUp
      const doneToday  = tasks.filter(t => getStatusCat(t) === 'done').length;
      ...
      // Pontos e conclusões da semana
      // Preferência: ClickUp ao vivo (doneToday/totalToday) > banco local (sync manual)
      ...
      // Se o ClickUp retornou tasks, usa contagem ao vivo; banco local só serve de fallback
      const doneSemana  = totalToday > 0
        ? doneToday  + Number(routineData.routine_done)
        : Number(wdb.done_semana) + Number(routineData.routine_done);
      const totalSemana = totalToday > 0
        ? totalToday
        : Number(wdb.total_semana);
      ...
      // Pts das tasks CONCLUÍDAS no ClickUp
      const ptsToday = tasks
        .filter(t => cuTaskDone(t))
        .reduce((sum, t) => sum + extractCuPts(t), 0);

      // Pts TOTAIS de todas as tasks da semana (feitas + em andamento)
      const ptsTotalSemana = tasks.reduce((sum, t) => sum + extractCuPts(t), 0);

      // Melhor fonte de pts para barra: banco local > concluídas ClickUp > total ClickUp
      const ptsParaBarra = ptsSemana || ptsToday || ptsTotalSemana;

      // Horas via time_spent com cap 16h/dia (seção 1 e 6) — agrupado por data de fechamento
      const HORA_CAP_DIA = 16;
      const dayHoras = {};
      for (const t of tasks.filter(t => cuTaskDone(t))) {
        const closedMs = Number(t.date_closed || t.date_done || 0);
